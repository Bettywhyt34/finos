...
function parseMappedRow(row: string[], colMap: ColumnMap): ParsedRow {
...
async function saveImport(accountId: string, rows: ParsedRow[]): Promise<ImportResult> {
...
export default function BankImportPage() {
  ...
  const [preparedRows, setPreparedRows] = useState<ParsedRow[]>([]);
  ...
  function buildPreview() {
    ...
    const allRows = rawRows.map((row) => parseMappedRow(row, colMap));
    setPreparedRows(allRows);
    setPreview(allRows.slice(0, 20));
  }

  async function handleImport() {
    if (!accountId) { toast.error("No bank account selected"); return; }
    if (preparedRows.length === 0) { toast.error("Build preview first"); return; }
    setImporting(true);
    const result = await saveImport(accountId, preparedRows);
    setImporting(false);
    ...
    toast.success(
      skipped > 0
        ? `${imported} new transaction${imported !== 1 ? "s" : ""} imported · ${skipped} duplicate${skipped !== 1 ? "s" : ""} skipped`
  ...
  const ColSelect = ({ field, label }: { field: keyof ColumnMap; label: string }) => (
    <div className="space-y-1.5">
      ...
      <Select value={colMap[field]} onValueChange={(v) => { setColMap((m) => ({ ...m, [field]: v ?? "" })); setPreview([]); setPreparedRows([]); }}>
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Import Bank Transactions</h1>
        <p className="text-sm text-slate-500 mt-1">Upload a CSV exported from your bank statement. FINOS will skip duplicate statement rows.</p>
      </div>
      ...
      {!rawRows.length ? (
        ...
        <div className="flex items-center gap-3 px-4 py-3 bg-green-50 border border-green-200 rounded-lg">
          ...
          <Button variant="ghost" size="icon-sm" onClick={() => { setRawRows([]); setHeaders([]); setPreview([]); setPreparedRows([]); setFileName(""); }}>
            <X className="h-4 w-4" />
          </Button>
        </div>
      )}
      ...
      {headers.length > 0 && (
        <div className="rounded-lg border border-slate-200 p-4 space-y-4 bg-white">
          ...
          <Button size="sm" onClick={buildPreview}>Prepare {rawRows.length} Rows</Button>
        </div>
      )}
      ...
      {preview.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-700">Preview — first {preview.length} of {preparedRows.length}</p>
              <p className="text-xs text-slate-400">All {preparedRows.length} prepared rows will be imported, not only the preview.</p>
            </div>
            <div className="flex items-center gap-1.5 text-green-700 text-xs"><CheckCircle2 className="h-3.5 w-3.5" /> Ready to import</div>
          </div>
          <div className="rounded-lg border border-slate-200 overflow-hidden text-xs">
            <table className="w-full">
              <thead className="bg-slate-50">
                <tr>{["Date", "Description", "Amount", "Type", "Reference"].map((h) => <th key={h} className="text-left px-3 py-2 font-medium text-slate-600">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {preview.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-3 py-1.5"><span className={cn("px-1.5 py-0.5 rounded-full", row.type === "CREDIT" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700")}>{row.type}</span></td>
          ...
          {!accountId && (
            <div className="flex items-center gap-2 text-amber-700 text-sm bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              <AlertCircle className="h-4 w-4 shrink-0" /> No bank account selected. Go back to a bank account and click Import CSV.
            </div>
          )}

          <Button onClick={handleImport} disabled={importing || !accountId}>
            {importing ? "Importing…" : `Import ${preparedRows.length} Transactions`}
          </Button>
        </div>
      )}
    </div>
