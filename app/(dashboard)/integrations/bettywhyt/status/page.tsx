...
import { DisconnectClientButton } from "./disconnect-button";
  ...
  if (!connection) {
    return (
      <div className="max-w-lg space-y-4">
        <h1 className="text-2xl font-bold text-slate-900">BettyWhyt Integration</h1>
        <div className="bg-white border border-slate-200 rounded-xl p-8 text-center space-y-4">
          <div className="w-12 h-12 rounded-full bg-pink-100 flex items-center justify-center mx-auto text-2xl">
            🌸
          </div>
          <div>
            <p className="font-semibold text-slate-800">Not connected</p>
            <p className="text-sm text-slate-500 mt-1">
              Connect BettyWhyt to sync inventory, online orders, and COGS into FINOS.
            </p>
          </div>
          <Link
            href="/integrations/bettywhyt/connect"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          { label: "Quarantined",  value: quarantineCount.toLocaleString() },
          { label: "Top Products", value: topItems.length.toLocaleString() + "+" },
        ].map((card) => (
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              These records failed to sync. Check account codes or run a full sync.
            </p>
          </div>
      ...
      {topItems.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <div className="px-6 py-3 border-b border-slate-200 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-700">Inventory Summary (top 10)</h2>
            <Link
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                {["SKU", "Name", "Online Qty", "POS Qty", "Reserved"].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50">
              {topItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2.5 text-right text-slate-700">{Number(item.qtyOnline).toLocaleString()}</td>
                  <td className="px-4 py-2.5 text-right text-slate-700">{Number(item.qtyPos).toLocaleString()}</td>
                  <td className="px-4 py-2.5 text-right text-amber-700">{Number(item.qtyReserved).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody className="divide-y divide-slate-50">
              {recentLogs.map((log) => {
                ...
                const duration = durationMs != null
                  ? durationMs < 60_000 ? `${(durationMs / 1000).toFixed(1)}s` : `${(durationMs / 60_000).toFixed(1)}m`
                  : "—";
      ...
      <div className="border-t border-slate-200 pt-4">
        <p className="text-xs text-slate-400">
          Disconnecting will stop all syncs and clear the stored API key. Existing FINOS records are kept.
        </p>
