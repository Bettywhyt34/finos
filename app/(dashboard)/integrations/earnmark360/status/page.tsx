  ...
  const [connection, recentLogs] = await Promise.all([
    ...
    prisma.syncLog.findMany({
      ...
      select: {
        ...
        startedAt: true, recordsProcessed: true, recordsCreated: true,
        recordsUpdated: true, recordsFailed: true, errorMessage: true,
      },
    }),
  ]);
  ...
  if (!connection) {
    return (
      <div className="max-w-lg space-y-4">
        <h1 className="text-2xl font-bold text-slate-900">EARNMARK360 Integration</h1>
        <div className="bg-white border border-slate-200 rounded-xl p-8 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Connect EARNMARK360 to sync payroll and HR data into FINOS.
            </p>
          </div>
          <Link
            href="/integrations/earnmark360/connect"
    ...
    <div className="space-y-6">
      ...
      {connection.status === "TOKEN_EXPIRED" && (
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
          ...
          <p className="text-sm text-orange-700 mt-1">Click Re-authorise to reconnect EARNMARK360.</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        {[
          ...
          {
            label: "Last Sync",
            value: connection.lastSyncAt ? new Date(connection.lastSyncAt).toLocaleString("en-NG") : "Never",
          },
