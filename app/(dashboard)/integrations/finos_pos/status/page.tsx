...
export default async function FinosPosStatusPage() {
  ...
  const [connection, recentLogs, quarantineCount, topItems] = await Promise.all([
    ...
    prisma.syncQuarantine.count({
      where: { tenantId, sourceApp: "finos_pos", resolved: false },
    }),
  ...
  if (!connection) {
    return (
      <div className="max-w-lg space-y-4">
        <h1 className="text-2xl font-bold text-slate-900">FINOS POS Integration</h1>
        <div className="bg-white border border-slate-200 rounded-xl p-8 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Connect FINOS POS to sync in-store sales, inventory, and COGS into FINOS accounting.
            </p>
          </div>
          <Link
            href="/integrations/finos_pos/connect"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          { label: "POS Products", value: topItems.length.toLocaleString() + "+" },
        ].map((card) => (
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              These POS records failed to sync. Check account codes IN-0011, OE-005, AS-002, CL-003 or run a full sync.
            </p>
          </div>
      ...
      {topItems.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <div className="px-6 py-3 border-b border-slate-200">
            <h2 className="text-sm font-semibold text-slate-700">POS Inventory (top 10 by qty)</h2>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                {["SKU", "Name", "POS Qty", "Online Qty", "Reserved"].map((h) => (
