    ...
    await prisma.$transaction(async (tx) => {
      ...
      const allocations = await tx.$queryRaw<DeferredAllocationRow[]>`
        SELECT
          ...
          (ila."unearned_created" - COALESCE(SUM(rria."amount"), 0)) AS "remaining"
        ...
        HAVING (ila."unearned_created" - COALESCE(SUM(rria."amount"), 0)) > 0.005
      ...
      if (available + 0.005 < amount) {
        throw new Error(
          `Only ${available.toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} NGN remains in Unearned Income for this Project.`,
        );
      }
      ...
      await tx.$executeRaw`
        INSERT INTO "project_revenue_recognitions" (
          ...
          "contract_asset_account_id", "journal_entry_id", "note", "created_by"
        ) VALUES (
          ${recognitionId}::uuid, ${tenantId}::uuid, ${project.id}, ${recognitionDate}, ${amount}, ${amount},
          0, 'NGN', ${primaryIncomeAccountId}, ${unearnedAccountId}, null, ${journalEntryId}, ${note}, ${userId}
        )
      `;
      ...
      for (const allocation of chosen) {
        await tx.$executeRaw`
          INSERT INTO "revenue_recognition_invoice_allocations" (
            ...
            ${tenantId}::uuid, ${recognitionId}::uuid, ${allocation.allocationId}::uuid, ${allocation.amount}
          )
        `;
      }
