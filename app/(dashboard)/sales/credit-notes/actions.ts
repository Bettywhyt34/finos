  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const invoice = await tx.invoice.findFirst({
        ...
        select: {
          id: true, customerId: true, invoiceNumber: true, issueDate: true, status: true,
          currency: true, exchangeRate: true, totalAmount: true, taxAmount: true,
          amountPaid: true, balanceDue: true, lines: { select: { projectId: true } },
        },
      });
      ...
      if (invoice.lines.some((line) => Boolean(line.projectId))) throw new Error("Project-linked invoice credit notes remain blocked until Project revenue-allocation adjustments are enabled.");
      ...
      const originalJournal = await tx.journalEntry.findFirst({ where: { tenantId, source: "invoice", sourceId: invoice.id, isLocked: true }, include: { lines: true } });
      ...
      const arBaseAmount = roundMoney(arAppliedAmount * exchangeRate);
      ...
      const usageRows = await tx.$queryRaw<Array<{ id: string; recognised: unknown; priorUnearnedCredit: unknown; priorRevenueCredit: unknown }>>`
        SELECT ila."id"::text AS "id",
          ...
          COALESCE((SELECT SUM(cnsa."unearned_reversed") FROM "credit_note_service_allocations" cnsa JOIN "credit_notes" cn ON cn."id"=cnsa."credit_note_id" WHERE cnsa."invoice_line_allocation_id"=ila."id" AND cn."status"='APPLIED'),0) AS "priorUnearnedCredit",
          COALESCE((SELECT SUM(cnsa."revenue_reversed") FROM "credit_note_service_allocations" cnsa JOIN "credit_notes" cn ON cn."id"=cnsa."credit_note_id" WHERE cnsa."invoice_line_allocation_id"=ila."id" AND cn."status"='APPLIED'),0) AS "priorRevenueCredit"
      ...
      const state = evidence.map((row) => {
        ...
        return { ...row, unearnedRemaining, revenueRemaining, serviceRemaining: roundMoney(unearnedRemaining + revenueRemaining), target: 0 };
      });

      for (const row of state) row.target = Math.min(row.serviceRemaining, roundMoney(row.invoiceAmount * ratio));
      let serviceDifference = roundMoney(serviceBaseAmount - state.reduce((sum, row) => sum + row.target, 0));
      ...
      if (Math.abs(roundMoney(state.reduce((sum, row) => sum + row.target, 0)) - serviceBaseAmount) > 0.01) throw new Error("The credit note service reduction exceeds the remaining invoice service value.");

      const serviceAllocations: Array<{ row: typeof state[number]; unearnedReversed: number; revenueReversed: number }> = [];
      ...
      for (const row of state.filter((item) => item.target > 0.005)) {
        ...
        if (unearnedReversed > 0.005) {
          ...
          journalLines.push({ accountId: row.unearnedIncomeAccountId, description: `Credit note - unearned service ${invoice.invoiceNumber}`, debit: unearnedReversed, credit: 0, reportingTags: row.reportingTags });
        }
        if (revenueReversed > 0.005) journalLines.push({ accountId: row.incomeAccountId, description: `Credit note - earned service ${invoice.invoiceNumber}`, debit: revenueReversed, credit: 0, reportingTags: row.reportingTags });
        serviceAllocations.push({ row, unearnedReversed, revenueReversed });
      }
      if (vatBaseAmount > 0.005) journalLines.push({ accountId: vatLines[0].accountId, description: `Credit note - Output VAT ${invoice.invoiceNumber}`, debit: vatBaseAmount, credit: 0 });
      if (arBaseAmount > 0.005) journalLines.push({ accountId: arLine.accountId, description: `Credit note - AR ${invoice.invoiceNumber}`, debit: 0, credit: arBaseAmount });
      if (customerCreditBaseAmount > 0.005) {
        const customerCreditAccount = await resolveSystemAccount(tx, tenantId, "CUSTOMER_CREDIT");
        journalLines.push({ accountId: customerCreditAccount.id, description: `Customer credit liability - ${invoice.invoiceNumber}`, debit: 0, credit: customerCreditBaseAmount });
      }
      ...
      const countRows = await tx.$queryRaw<Array<{ count: bigint }>>`SELECT COUNT(*)::bigint AS "count" FROM "credit_notes" WHERE "tenant_id"=${tenantId}::uuid`;
      ...
      const journalEntryId = await postJournalEntryInTransaction(tx, {
        tenantId, createdBy: userId, entryDate: issueDate, reference: creditNumber,
        description: `Credit note ${creditNumber} against ${invoice.invoiceNumber}: ${reason}`,
        recognitionPeriod: getRecognitionPeriod(issueDate), source: "credit_note", sourceId: creditNoteId, lines: journalLines,
      });

      await tx.$executeRaw`
        INSERT INTO "credit_notes" (
          "id","tenant_id","customer_id","credit_number","invoice_id","issue_date","amount","reason","status","currency","exchange_rate","base_amount",
          "ar_applied_amount","customer_credit_amount","service_base_amount","vat_base_amount","journal_entry_id","applied_at","created_by"
        ) VALUES (
          ...
          ${arAppliedAmount},${customerCreditAmount},${serviceBaseAmount},${vatBaseAmount},${journalEntryId},now(),${userId}
        )
      `;
      ...
      for (const allocation of serviceAllocations) {
        await tx.$executeRaw`
          INSERT INTO "credit_note_service_allocations" ("tenant_id","credit_note_id","invoice_line_allocation_id","service_base_amount","unearned_reversed","revenue_reversed")
          VALUES (${tenantId}::uuid,${creditNoteId},${allocation.row.id}::uuid,${allocation.row.target},${allocation.unearnedReversed},${allocation.revenueReversed})
        `;
      }
