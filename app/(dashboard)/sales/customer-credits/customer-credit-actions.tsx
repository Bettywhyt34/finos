...
export function CustomerCreditActions({ credit, invoices, bankAccounts }: { credit: CustomerCreditView; invoices: CreditInvoiceOption[]; bankAccounts: CreditBankOption[] }) {
  ...
  function apply() {
    if (!invoiceId || !selectedInvoice) return toast.error("Select an invoice");
    ...
    startTransition(async () => {
      ...
      toast.success("Customer credit applied to invoice");
      setApplyOpen(false);
      router.refresh();
    });
  }
  ...
  function refund() {
    if (!bankId) return toast.error("Select a refund bank account");
    ...
    startTransition(async () => {
      ...
      if ("error" in result) return toast.error(result.error);
      toast.success("Customer credit refunded");
      setRefundOpen(false);
      router.refresh();
    });
  }
    ...
    <div className="flex justify-end gap-2">
      ...
      <Dialog open={applyOpen} onOpenChange={setApplyOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Apply customer credit</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="rounded-md bg-[var(--surface-muted)] p-3 text-xs text-[var(--text-secondary)]">Available: <span className="font-semibold text-[var(--text-primary)]">{formatCurrency(credit.remainingAmount, credit.currency)}</span>. This reduces AR; it does not count as cash received.</div>
            <div className="space-y-1.5"><Label>Invoice</Label><Select value={invoiceId} onValueChange={(value) => { setInvoiceId(value ?? ""); const inv=eligibleInvoices.find((item)=>item.id===value); if(inv) setApplyAmount(Math.min(credit.remainingAmount,inv.balanceDue)); }}><SelectTrigger><SelectValue placeholder="Select invoice" /></SelectTrigger><SelectContent>{eligibleInvoices.map((invoice)=><SelectItem key={invoice.id} value={invoice.id}>{invoice.invoiceNumber} · {formatCurrency(invoice.balanceDue, invoice.currency)} outstanding</SelectItem>)}</SelectContent></Select></div>
            <div className="grid grid-cols-2 gap-3"><div className="space-y-1.5"><Label>Date</Label><Input type="date" value={applicationDate} onChange={(e)=>setApplicationDate(e.target.value)} /></div><div className="space-y-1.5"><Label>Amount</Label><Input type="number" min="0.01" step="0.01" value={applyAmount} onChange={(e)=>setApplyAmount(Number(e.target.value)||0)} /></div></div>
          </div>
      ...
      <Dialog open={refundOpen} onOpenChange={setRefundOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Refund customer credit</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5"><Label>Refund from</Label><Select value={bankId} onValueChange={(value)=>setBankId(value??"")}><SelectTrigger><SelectValue placeholder="Select bank account" /></SelectTrigger><SelectContent>{eligibleBanks.map((bank)=><SelectItem key={bank.id} value={bank.id}>{bank.bankName} · {bank.accountName}</SelectItem>)}</SelectContent></Select></div>
            <div className="grid grid-cols-2 gap-3"><div className="space-y-1.5"><Label>Date</Label><Input type="date" value={refundDate} onChange={(e)=>setRefundDate(e.target.value)} /></div><div className="space-y-1.5"><Label>Amount</Label><Input type="number" min="0.01" max={credit.remainingAmount} step="0.01" value={refundAmount} onChange={(e)=>setRefundAmount(Number(e.target.value)||0)} /></div></div>
            {credit.currency !== "NGN" ? <div className="space-y-1.5"><Label>Refund exchange rate (NGN per {credit.currency})</Label><Input type="number" min="0.000001" step="0.000001" value={refundRate} onChange={(e)=>setRefundRate(Number(e.target.value)||0)} /></div> : null}
          ...
          <DialogFooter><DialogClose render={<Button variant="outline" type="button" />}>Cancel</DialogClose><Button type="button" disabled={pending} onClick={refund}>{pending?"Refunding…":"Refund credit"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
