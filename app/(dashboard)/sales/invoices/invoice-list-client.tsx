...
type InvoiceRow = {
  ...
  sentAt: Date | null
  paidAt: Date | null
  convertedFromAcceptedQuote: boolean
...
export function InvoiceListClient({ invoices }: { invoices: InvoiceRow[] }) {
  ...
  const selectedDeletableCount = selectedRows.filter((invoice) => invoice.status === "DRAFT" && !invoice.convertedFromAcceptedQuote).length
  const selectedProtectedCount = selectedRows.filter((invoice) => invoice.status === "DRAFT" && invoice.convertedFromAcceptedQuote).length

  const handleDelete = () => {
    const ids = Array.from(selected)
    startTransition(async () => {
      const result = await bulkDeleteInvoicesSafely(ids)
      setDeleteOpen(false)
  ...
  const handlePostToLedger = () => {
    ...
    startTransition(async () => {
      ...
      if ("error" in result) { toast.error(result.error); return }
      setSelected(new Set())
      if (result.errors.length === 0) toast.success(`${result.posted} invoice${result.posted !== 1 ? "s" : ""} posted to ledger`)
      else toast.warning(`${result.posted} posted, ${result.errors.length} failed`)
    })
  }
  ...
  return (
    <div className="space-y-3">
      {selected.size > 0 && (
        <div className="flex items-center justify-between px-4 py-2.5 rounded-lg bg-emerald-50 border border-emerald-200">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setSelected(new Set())} disabled={pending} className="h-7 text-xs border-emerald-300 text-emerald-700 hover:bg-emerald-100">Clear</Button>
            <Button variant="outline" size="sm" onClick={() => setDeleteOpen(true)} disabled={pending || selectedDeletableCount === 0} className="h-7 text-xs border-red-300 text-red-600 hover:bg-red-50 gap-1.5"><Trash2 className="h-3.5 w-3.5" /> Delete</Button>
            <Button size="sm" onClick={handlePostToLedger} disabled={pending} className="h-7 text-xs bg-emerald-600 hover:bg-emerald-700 text-white gap-1.5">
              {pending ? <><Loader2 className="h-3.5 w-3.5 animate-spin" /> Posting…</> : <><BookOpen className="h-3.5 w-3.5" /> Post to Ledger</>}
            </Button>
          </div>
        </div>
      )}
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Delete {selectedDeletableCount} draft invoice{selectedDeletableCount !== 1 ? "s" : ""}?</DialogTitle></DialogHeader>
          <p className="text-sm text-slate-600">Only ordinary DRAFT invoices will be deleted. Accepted-quote drafts are protected and remain available for posting.</p>
          {selectedProtectedCount > 0 && <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-3 py-2">{selectedProtectedCount} selected draft{selectedProtectedCount !== 1 ? "s are" : " is"} linked to an accepted quote and will be skipped.</p>}
          <DialogFooter>
            <DialogClose render={<Button variant="outline" type="button" disabled={pending} />}>Cancel</DialogClose>
            <Button variant="destructive" onClick={handleDelete} disabled={pending || selectedDeletableCount === 0} className="gap-1.5">{pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
