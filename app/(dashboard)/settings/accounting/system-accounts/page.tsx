...
const ROLES: Array<{
  ...
  { role: "UNEARNED_REVENUE", label: "Unearned Revenue", description: "Customer consideration received or billed before revenue is earned.", types: ["LIABILITY"] },
];
...
export default async function SystemAccountsPage() {
  ...
  const [accounts, mappings] = await Promise.all([
    ...
    prisma.$queryRaw<MappingRow[]>`
      SELECT "role", "account_id" AS "accountId"
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      <header>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--text-secondary)]">
          Map FINOS accounting roles to your Chart of Accounts. These mappings control where automatic journals post; account codes remain your organisation&apos;s choice.
        </p>
      </header>

      <div className="overflow-hidden rounded-xl border border-[var(--app-border)] bg-white">
        <div className="grid grid-cols-[1.25fr_1.5fr_auto] gap-4 border-b border-[var(--app-border)] bg-[var(--surface-muted)] px-5 py-3 text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)]">
          <span>Accounting role</span>
            ...
            return (
              <form key={item.role} action={saveSystemAccountMapping} className="grid grid-cols-[1.25fr_1.5fr_auto] gap-4 px-5 py-4 items-center">
                ...
                <select
                  ...
                  className="h-10 w-full rounded-md border border-[var(--app-border)] bg-white px-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--finos-accent)]"
                >
                ...
                <button
                  type="submit"
                  className="h-10 rounded-md bg-[var(--finos-accent)] px-4 text-sm font-semibold text-white transition-opacity hover:opacity-90"
                >
                  Save
                </button>
              </form>
      ...
      <p className="text-xs leading-5 text-[var(--text-secondary)]">
        FINOS validates each mapping by account type before saving. Automatic postings will stop rather than use the wrong account when a required mapping is missing.
      </p>
    </div>
  );
}
